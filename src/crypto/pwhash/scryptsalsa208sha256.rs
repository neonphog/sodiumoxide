...
pub fn derive_key<'a>(key: &'a mut [u8], passwd: &[u8], salt: &Salt,
...
pub fn pwhash(passwd: &[u8], OpsLimit(opslimit): OpsLimit,
    ...
    if unsafe {
        ffi::crypto_pwhash_scryptsalsa208sha256_str(hp.0.as_mut_ptr() as *mut _,
                                                    passwd.as_ptr() as *const _,
                                                    passwd.len() as c_ulonglong,
                                                    opslimit as c_ulonglong,
                                                    memlimit)
...
pub fn pwhash_verify(hp: &HashedPassword,
                     passwd: &[u8]) -> bool {
    unsafe {
        ffi::crypto_pwhash_scryptsalsa208sha256_str_verify(hp.0.as_ptr() as *const _,
                                                           passwd.as_ptr() as * const _,
                                                           passwd.len() as c_ulonglong)
            == 0
    }
